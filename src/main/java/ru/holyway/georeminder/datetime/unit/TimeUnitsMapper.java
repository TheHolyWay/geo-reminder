...
public class TimeUnitsMapper {
    ...
    public TimeUnitsMapper(Lemmatizer lemmatizer) {
    ...
    public TimeOfDay resolveTimeOfDay(String word) {
        String lemma = lemmatizer.resolveLemma(word.toLowerCase());

        Optional<TimeOfDay> timesOfDay = timesOfDayMap.entrySet().stream()
                .filter(e -> e.getKey().contains(lemma) || e.getKey().contains(word))
